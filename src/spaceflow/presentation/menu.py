...
def _draw_menu() -> None:
    ...
    print(paint("  CONTROL DE AUDIO", BOLD, DIM, CYAN))
    _option("3", "Pausar audio", GREEN)
    _option("4", "Continuar audio", GREEN)
    _option("5", "Bajar volumen (-10)", YELLOW)
    _option("6", "Subir volumen (+10)", YELLOW)
    _option("7", "Detener audio", RED)
    print(paint("  GUARDAR", BOLD, DIM, CYAN))
    _option("8", "Descargar grabación", GREEN)
    _option("9", "Grabar un Space en vivo", GREEN)
    _option("B", "Ver biblioteca", GREEN)
    print(paint("  HERRAMIENTAS", BOLD, DIM, YELLOW))
    _option("C", "Importar cookies de X (opcional)", YELLOW)
    _option("U", "Buscar actualizaciones", YELLOW)
    _option("A", "Configuración", YELLOW)
    _option("0", "Salir", RED)
    ...
    while True:
        ...
        try:
            ...
            if choice == "1":
                ...
                print(warning("Usa 3-7 para pausar, cambiar volumen o detenerlo."))
            elif choice == "3":
